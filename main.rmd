...
limit_summary <- games_split %>%
  ...
  summarise(
    TotalMatches = n(),
    Reached = sum(ReachedLimit, na.rm = TRUE),
    ProportionReached = Reached / TotalMatches
  ) %>%
  arrange(desc(ProportionReached))
...
aes(x = reorder(GameType_clean, ProportionReached),
y = ProportionReached)) +
geom_col() +
coord_flip() +
labs(
...
numeric_vars <- games_split %>% select(where(is.numeric))
...
model_data <- games_split %>% select(TotalXP, all_of(colnames(numeric_vars))) %>% drop_na()
...
full_model <- lm(TotalXP ~ ., data = model_data)
summary(full_model)
...
significant_predictors <- significant_predictors[significant_predictors[,4] < 0.05, ]
