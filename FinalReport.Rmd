...
In past mini-projects related to Call of Duty matches, we experimented with creating models to predict a match's TotalXP - the player's number of experience points gained within each match based on the player's match performance. In this section, we will further build a model predicting a player's TotalXP in a match and highlight the most important predictors. This process requires additional data wrangling and model selection.
